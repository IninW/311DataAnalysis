...
t = data %>%
  ...
  summarise(ave = mean(handle))
ggplot(t, aes(x = reorder(RequestSource, ave), y = ave)) +
  geom_bar(stat = 'identity', alpha = 0.5, fill = c(rep('#006d2c', 2), 
                                                    rep('#045a8d', 6))) +
  ggtitle('')+
  ylab('') +
  xlab('') +
...
ggplot(t, aes(x = week, y = aves)) +
  geom_line(size = 1.5, color = '#016c59') +
  geom_area(aes(x = week, y = counts), fill = '#74a9cf', alpha = 0.4) +
  scale_x_continuous(breaks = seq(0, 60, 20), labels = c('Aug-2015', 'Dec-2015', 'May-2016', 'Sep-2016')) +
...
t$RequestType = factor(t$RequestType, labels = c('bulky', 'deadAnimal', 'electronic', 'feedback', 'graffiti', 'homeless', 'dumping', 'appliance', 'streetlighM', 'other', 'water', 'streetlightS'))
t1 = t %>%
  select(RequestType, aves) %>%
  mutate(class = 'HandleTime')
t2 = t %>%
  select(RequestType, counts) %>%
  mutate(class = 'RequestNumber')
colnames(t2) = c('RequestType', 'aves', 'class')
t = rbind(t1, t2)
ggplot(t, aes(x = RequestType, y = aves, group = class, fill = class)) +
...
# heatmap efficiency and frequancy
# weekday and hour
h = data %>%
  filter(year == 2016) %>%
...
# individual one for highlight in slides
t = data %>%
...
t$counts = (t$count-min(t$count))/(max(t$count)-min(t$count))
t1 = t %>%
  select(RequestType, Owner, aves) %>%
  mutate(class = 'HandleTime')
t2 = t %>%
  select(RequestType, Owner, counts) %>%
  mutate(class = 'RequestNumber')
colnames(t2) = c('RequestType','Owner', 'aves', 'class')
t = rbind(t1, t2)
ggplot(t, aes(x = Owner, y = aves, group = class, fill = class)) +
  geom_bar(stat = 'identity', position = position_dodge(1)) +
  scale_fill_manual(values = c('pink', 'lightblue'), 
                    name = '') +
  theme(axis.text.y = element_blank()) +
  ylab('Request number/handeling time') +
  facet_wrap(~RequestType, nrow = 3,  scale = 'free_y') +
  theme(axis.title.x=element_text(size=18,face="bold",family="Times"),
        axis.title.y=element_text(size=18,face="bold",family="Times"),
        axis.text=element_text(family="Times",size=14),
        legend.title=element_text(face="bold",size=16,family="Times"),
